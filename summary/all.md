...
STL是数据结构和算法的精妙实现，因此为了追求极致的效率，内部并没有给所有的操作加锁。对于STL的容器而言，只有下面这两种情况可以保证线程安全：
...
void *OSMemoryImp::Hook_New(size_t nSize, bool bArray, NEW_FUNCTION_SRC eumFunSrc)
{
    ...
    // 开始进行拷贝,首先拷贝HEAD
    unsigned int *pHeader = (unsigned int *)pMem;
    *pHeader = HEADER;
    // 拷贝结尾
    unsigned int* pTail = (unsigned int*)(pMem + nTotalAllocateSize - sizeof(TAIL));
    *pTail = TAIL;

    LP_MEMORY_META_DATA pMemMetaDataInfo = (LP_MEMORY_META_DATA)(pMem + sizeof(HEADER));
    if (NULL == pMemMetaDataInfo)
    {
        // 释放已经申请的内存
        free(pMem);
        printf("##################################In [%s] get pMemMetaDataInfo failed!!!!!!!!!!!!!!!!\n",__FUNCTION__);
        return NULL;
    }

    // 记录申请的内存时间
    //pMemMetaDataInfo->AllocateTime = time(NULL);
    pMemMetaDataInfo->nObjSize = nSize;
    pMemMetaDataInfo->pAddress = pMem + OFFSET;
    pMemMetaDataInfo->nUserCustomNew = ALLOCATE_STD_MODE;
    pMemMetaDataInfo->nFunctionSrc = eumFunSrc;
    //如果是申请数组的
    if (bArray)
    {
        pMemMetaDataInfo->nMemoryType = MEMORY_TYPE_ARRAY;
    }
    else
    {
        pMemMetaDataInfo->nMemoryType = MEMORY_TYPE_OBJ;
    }
...
在该项目中首先对提高流媒体服务质量进行需求分析，将流媒体服务质量分为3大块，源头质量、交互质量以及感官质量，而我们产品在之前最大的痛点就是没有度量，就难以优化。其中交互质量的度量在上一个项目中已经得到度量，而本项目主要解决的是源头质量的度量问题。源头质量主要是媒体文件也就是ts的健康指标，IPTV使用TS作为封装层，TS流包含了各种用于视频流解码所必须的信息内容，因此ts流的健康状况检测对保障IPTV用户体验质量十分重要。TR101-290标准对TS流的健康指标进行了详细的定义，需按其标准对内容提供者进行检测。确立需求后，开始寻找解决方案。我是有之前解决UT覆盖率的经验后，首先是寻找解决该问题的优质开源代码，最后从三个开源代码中选择了easyice这家开源工具，因为首先它的开源许可证是MIT，可以直接拿来迭代会进行商用，其次其不仅实现了TR101-290的三个等级检测，而且留了es层的检测接口，方便我们之后对不同编码类型的媒体流进行pts/dts的检测。我在项目中对其进行了再次封装，调用其接口对媒体文件进行解析，在根据自己对媒体文件的检测需求，分析出结果，是否为合格ts，整个代码再封装成静态库，在我们的内容发布之后，对媒体文件进行检测，并对非法结果进行上报告警。

## 8.4 流媒体起播优化
TS流（transport stream）用于相对有错环境下的传输与存储（如DVB、IPTV中），其基本单位是TS包，长度固定188字节。TS流由多种数据组合而成，一个TS包中的数据可以是视频数据、音频数据、填充数据、PSI/SI表格等，它们都有唯一的PID对应。
解码的顺序：
1.  根据TS流头文件信息中的PID（PAT的PID值为0）获得PAT表，TS流中可能存在一个或多个节目，而PAT信息指出TS流中存在的所有节目。根据PAT表可以获得所有节目的PMT表的PID，根据这个PID可以从TS流中找到PMT表。
2.  根据节目1的PMT表的PID（例如0x20），在接收的TS流中找到该节目的PMT表，PMT表里存储的是携带该节目音视频流的TS的PID。
3.  获得节目1中的音视频流的PID后，就可以从接收的TS流中通过解复用器的过滤器，将节目1的音视频流TS筛选出来，将TS包中的Payload数据取出，拼接成一个个PES包，每个PES包都是I、P、B帧的一种。
4.  根据拼接获取到的PES，按照PES包头信息中的DTS（解码时间标签）顺序进行解码。这里需要注意的是，I帧必须先解码，因为I帧中包含完整图像的所有信息，然后解码P帧，P帧是前向预测帧，根据之前的I帧或解码后的其他P帧进行解码，最后解码B帧，B帧是双向预测帧，不仅依赖它之前的帧还依赖它之后的帧信息。

如何解决该问题：
1. 首先在内容发布流程中，存储PAT/PMT包在索引文件中
2. 在服务过程，首先从全局索引文件中先读出PAT/PMT包
3. 发包定位到索引文件中的I帧
4. 发包的时候先发PAT/PMT，然后发送I帧
## 8.5 UT
...
C语言调用函数顺序：每个函数调用都会分配函数栈。在栈内进行函数执行过程。调用前，先把返回地址压栈，然后把当前函数的栈指针（esp）压栈。函数中参数的压栈顺序是从右到左

这里用到了两个寄存器，`%ebp`是帧指针，它总是指向当前帧的底部；`%esp`是栈指针，它总是指向当前帧的顶部
